...
def admin():
    if request.method == 'POST':
        ...
        if jenis_layanan_selected == 'all':
            datadiris = db.DataSkm.find()  # Memuat semua data jika dipilih "Semua Jenis Layanan"
        else:
            datadiris = db.DataSkm.find({'layanan': jenis_layanan_selected})  # Memuat data sesuai dengan jenis layanan yang dipilih
        return render_template('table_rows.html', datadiris=datadiris,)  # Mengirim kembali data tabel yang diperbarui
    else:
        datadiris = db.DataSkm.find()  # Mengambil semua data dari koleksi datadiri
...
def get_data(filtered=True):
    if filtered:
        three_months_ago = datetime.now() - timedelta(days=90)
        data = list(db.DataSkm.find({}, {'_id': 0}))
        data_filtered = [item for item in data if parse_date(item['waktu_dibuat']) >= three_months_ago]
    else:
        data_filtered = list(db.DataSkm.find({}, {'_id': 0}))
    ...
    for item in data_filtered:
        ...
        total_skor.append(item.get('total_skor', 0))

    return jenis_kelamin_count, layanan_count, total_skor

@app.route('/statistik_dashboard')
def statistik_dashboard():
    filtered = request.args.get('filtered', 'true').lower() == 'true'
    jenis_kelamin_count, layanan_count, total_skor = get_data(filtered)
    return render_template('statistik_dashboard.html', jenis_kelamin_count=jenis_kelamin_count, layanan_count=layanan_count, total_skor=total_skor)

@app.route('/get_filtered_data')
def get_filtered_data():
    filtered = request.args.get('filtered', 'true').lower() == 'true'
    jenis_kelamin_count, layanan_count, total_skor = get_data(filtered)
    return jsonify(jenis_kelamin_count=jenis_kelamin_count, layanan_count=layanan_count, total_skor=total_skor)
...
def export_excel():
    ...
    data = list(db.DataSkm.find(query, {'_id': 0}))
